...
import org.tensorflow.lite.examples.detection.YoloActivity;
...
public class YoloV4Classifier implements Classifier {
    ...
    protected ByteBuffer convertBitmapToByteBuffer(Bitmap bitmap) {
        int SIZE = 4 * BATCH_SIZE * INPUT_SIZE * INPUT_SIZE * PIXEL_SIZE;
        ...
        for (int i = 0; i < gridWidth;i++){
            ...
            for (int c = 0;c<labels.size();++c){
                if (classes[c] > maxClass){
                    detectedClass = c;
                    maxClass = classes[c];
                    Log.i("[maxClass] :", String.valueOf(maxClass));
